...
import {ProgramDetail} from "@kit-iai-proof/proof-config-manager-client";
import UploadPanel from "../components/UploadPanel.tsx";
import {ConfigContext} from "../../../provider/IConfigContext.tsx";

const ProgramConfigsImport: () => ReactNode = (): ReactNode => {

    const {t} = useTranslation();
    const navigate: NavigateFunction = useNavigate();
    const {updateProgramId, programMutation} = useContext(ConfigContext);
    const [value, setValue] = useState<ProgramDetail | undefined>(undefined);
    ...
    const handleChange: (e: any) => void = (e: any) => {
        ...
        fileReader.onload = ({target}): void => {
            if (target) {
                ...
                if (content) {
                    const value: ProgramDetail = JSON.parse(content);
                    setValue(value);
                }
            }
        };
                                ...
                                <Fragment>
                                    ...
                                    <Tooltip title={t("action.importProgram")}>
                                        <Button
                                            ...
                                            onClick={async (): Promise<void> => {
                                                if (value) {
                                                    await programMutation.mutateAsync(value)
                                                }
                                            }}
                                            color={"primary"}>
                                            {t("action.import")}
...
export default ProgramConfigsImport;